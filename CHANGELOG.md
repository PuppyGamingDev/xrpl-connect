...
- New `@xrpl-connect/adapter-joey` adapter for Joey Wallet. Joey is WalletConnect-based, but connects via Joey's own SDK (`@joey-wallet/wc-client`) so it targets Joey directly through the SDK's `generate()` action - deep-linking straight into the Joey app on mobile, and rendering a Joey-specific QR code on desktop - rather than opening the generic multi-wallet WalletConnect picker. `JoeyAdapter.signMessage()` throws `WalletErrorCode.UNSUPPORTED_METHOD`, since Joey's SDK only exposes transaction-signing methods, with no message-signing equivalent. `packages/ui`'s `WalletService` gained a dedicated `joey` connection branch (mirroring the existing `walletconnect` one) so `<xrpl-wallet-connector>` renders Joey's QR/deeplink flow out of the box. `connect()` derives the account's network from the chain the wallet actually approves (parsed out of the returned CAIP-10 account string) rather than assuming it matches the requested network 1:1, and clears any WalletConnect session auto-restored from persisted storage before pairing - both the request and any restored session could otherwise silently disagree with the network the app actually asked for, and once approved with the wrong chain every `sign()`/`signAndSubmit()` call gets rejected by the WalletConnect session as "Missing or invalid". **`JoeyAdapter` needs its own WalletConnect/Reown project ID, distinct from any other WalletConnect client (e.g. `WalletConnectAdapter`) in the same app** - Joey's SDK builds its own independent WalletConnect client, and reusing a project ID makes two independent clients collide over the same WalletConnect "Core" (relay + session storage), which manifests as `Init() was called N times` console warnings and connections silently landing on the wrong network or a stale session; documented in the adapter README and `docs/guide/getting-started.md`, and the vanilla-js example now uses (and warns about) a separate `JOEY_WALLETCONNECT_PROJECT_ID`. `connect()` also now guards against overlapping calls on the same adapter instance - most commonly `WalletManager`'s `autoConnect` silently reconnecting stored state in the background while the user also clicks "Connect" - by reusing the in-flight pairing (and forwarding the newer call's `onQRCode` to it) instead of starting a second, competing WalletConnect negotiation that could get approved instead of the one the user actually scanned. The adapter now also listens for the underlying WalletConnect session's `session_update` event (e.g. the user switches network inside the Joey app without disconnecting) and re-syncs the connected account, emitting `networkChanged`/`accountChanged` as appropriate instead of silently signing against a stale chain/address.
